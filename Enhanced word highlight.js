...
//{	values >
	...
	var STYLE_COLOR = ['#ff99cc','#66cc66','#cc99ff','#ffcc66','#66aaaa','#dd9966','#aaaaaa','#dd6699','#FFFF80','#99ccff'];
	var BORDER_COLOR = ['#aaaa20','#4477aa','#aa4477','#117711','#7744aa','#aa7711','#115555','#884411','#555555','#881144'];
	var STYLE_COLOR_2 = ['#ffbbcc','#88ee88','#ccbbff','#ffee88','#88cccc','#ffbb88','#cccccc','#ffaabb','#FFFFa0','#bbeeff'];
	...
	//language detection
	if ((navigator.userAgent.toLowerCase().indexOf('ru-') == -1) || ((navigator.userAgent.toLowerCase().indexOf('firefox') != -1) && (navigator.language.indexOf('ru-') == -1))) {
		_L = 0;
	} else _L = 1;
	var _L = (!!(navigator.userAgent.toLowerCase().indexOf('ru-') == -1))? 0:1;
	if(navigator.userAgent.toLowerCase().indexOf('firefox') != -1) 
	{_L = (!!(navigator.language.indexOf('ru-') == -1))? 0:1;} // Thanks to SoIN(http://userscripts.org/users/302257)
	var _ti = { // en/zh locale string for tooltip.
		edit: ['Edit current keywords','Редактировать слова'],
		edit_a: ['Confirm editing keywords','Подтвердить редактирование слов'],
		off:  ['Toggle all keywords\' highlight','вкл.выкл. подсветки'],
		td0:  ['Double-click to minimize the panel','Двойной клик - минимизировать панель'],
		td0_a:  ['Double-click to restore EWH panel','Двойной клик - восстановление панели'],
		lock: ['Lock current set of keywords','Заблокировать текущий набор слов'],
		lock_a: ['Current locked keyword(s):','Заблокированные на данный момент слова：'],
		lock_u: ['Function not supported by this browser','Функция не поддерживается браузером'],
		close: ['Close Enhanced word highlight','Закрыть'],
		kwL:  ['Left click to the next; Right click to the previous','Левый клик - следующее; ПКМ - предыдущее'],
		check: [['Toggle highlight of "','"'],['切换“','”的高亮']],
		mapl:['Toggle highlight map locking status','Toggle highlight map locking status'],
		ad_nw: ['Toggle add/new keywords for highlight','Toggle add/new keywords for highlight'],
		subm: ['Submit keywords','Подтвердить слова'],
		clos: ['Close input box','Закрыть место для ввода']
	};
...
// #### Config I #### --------------------------{{
	...
	// keybinds
	var KEY_NEXT = 'C-m';		// "n"			Next occurrence
...
// #### Config II #### --------------------------{{
	...
	// save panel position?
	var save_panel_pos = true;
	
	// show indicator bar when navigating?
	var show_indc_bar = true;
// #### Config II #### --------------------------}}
//}
...
// Functions
	...
	function highlight(doc, ext_word) {
		...
		if (_words.length === 1 && _words[0].exp) {
			...
			xw = ' and (' + _words.map(function(w){return ' contains(translate(self::text(),"abcdefghijklmnopqrstuvwxyzабвгдеёжзийклмнопрстуфхцчшщъыьэюя","ABCDEFGHIJKLMNOPQRSTUVWXYZАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ"),'+escapeXPathExpr(w.toUpperCase())+') ';}).join(' or ') + ') ';
		}
	...
	function addsheet() {
		...
		var hilistyles = STYLE_COLOR.map(function(rgb,i){
			return 'span.' + PRE + '_word'+i+',.' + PRE + '_item'+i+'.strike{box-shadow:inset 0 -2px '+rgb+'!important;}';});
		var borderstyles = BORDER_COLOR.map(function(rgb,i){
			return 'li.' + PRE + '_item'+i+'{outline:1px solid '+rgb+'!important;}';
			});
		sheet = addCSS([
			//Additional Style
			'span[class^="' + PRE + '_word"]{font:inherit!important;important;margin:3!important;padding:0!important;text-align:inherit!important;float:none!important;position:static!important;}', //vertical-align:inherit !important;
			'#' + PRE + '_words, #' + PRE + '_words *{font-family: Arial ;}',
			'#' + PRE + '_words{line-height:1;position:fixed;z-index:60000;opacity:0.5;list-style-type:none;margin:0;padding:0;width:auto;max-width:100%;' + panel_pos_arr[0] + panel_pos_arr[1] +'}',
...
//			aside.style.width = Math.max(320,_aside_w) +'px';
	...
	function instant_search(_r, e_target) {
		...
		input.addEventListener('input',function(e) {
			...
			var x = 'descendant::text()[contains(translate(self::text(),"abcdefghijklmnopqrstuvwxyzабвгдеёжзийклмнопрстуфхцчшщъыьэюя","ABCDEFGHIJKLMNOPQRSTUVWXYZАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ"),'+escapeXPathExpr(text)+') and not(ancestor::textarea) and not(ancestor::script) and not(ancestor::style)]/parent::*';
